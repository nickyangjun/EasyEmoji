...
public class LovedEmojiFragment<T extends Emoji> extends EmojiFragment {
    private LovedPageEmojiStyle<T> emojiconData;
    private RecyclerView mRecyclerView;
    private EmojiAdapter mEmojiAdapter;

    @Override
    public void setData(PageEmojiStyle emojiData) {
        emojiconData = (LovedPageEmojiStyle<T>) emojiData;
    }
    ...
    private class EmojiAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
            ...
            if(emojiHolder.type==DELETE){
                ...
                Emoji emoji= emojiconData.getData().get(position);
                (emojiHolder.icon).setText(emoji.getEmoji());
            }
                ...
                public void onClick(View v) {
                    if(mOnEmojiconClickedListener != null){
                        if(position == getItemCount() -1){
                            ...
                            mOnEmojiconClickedListener.onEmojiconClicked(emojiconData.getData().get(position).emoji);
                        }
                    }
                }
            });
